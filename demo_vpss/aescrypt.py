...
def encrypt(infile, outfile, password, key_size=32, salt_marker=SALT_MARKER,
    ...
    file_content = header + salt
    file_content = file_content + iv
    ...
    while not finished:
        ...
        if len(chunk) == 0 or len(chunk) % bs != 0:
            ...
            finished = True
        file_content = file_content + cipher.encrypt(chunk)
        #outfile.write(cipher.encrypt(chunk)) 

    #outfile.write(file_content) 
    outfile.write(hexlify(file_content)) 
...
def decrypt(infile, outfile, password, key_size=32, salt_marker=SALT_MARKER,
    ...
    """
    file_content = unhexlify(infile.read())
    ...
    while not finished:
        chunk, next_chunk = next_chunk, cipher.decrypt(file_content[i:i+1024*bs])
        i = i + 1024*bs
...
def main(args=None):
    ...
    with open(args.infile, 'rb') as infile, \
        ...
        if args.decrypt:
            ...
            if password == "USER_KEY_DOES_NOT_SATISFY_POLICY":
                print (password)
            else:
