...
def main(args=None):
    ...
    # must be the current working direcroty path and the name of the file
    ap.add_argument('password', nargs='?', help="Password file")
    ap.add_argument('usr', nargs='?', help="User name")
    ...
    """
    userfolder = os.getcwd() + "/users/" + args.usr
    plain_file_path = userfolder + "/plaintexts/" + args.infile
    cipher_file_path = userfolder + "/ciphertexts/" + args.infile + '.enc'
    password = userfolder +  "/secret_keys/AES_sk/" + args.password

    # get password from <CURRENT_WORKING_DIRECTORY>/<LOCAL_PATH>/symmetric.k file
    with open(password ,"r") as pwd_file:
    ...
    if args.decrypt:
        with open(cipher_file_path, 'rb') as infile, \
            open(plain_file_path, 'wb') as outfile:
                #decrypt(infile, outfile, getpass("Enter decryption password: "))
                # first argument is input file, second argoment is output file
                print ("INFILE:\n%s\n" % infile)
                print ("OUTFILE:\n%s\n" % outfile)
                print ("PASSWORD:\n%s\n" % password)
        ...
        """
        with open(plain_file_path, 'rb') as infile, \
            open(cipher_file_path, 'wb') as outfile:
