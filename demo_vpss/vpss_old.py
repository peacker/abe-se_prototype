...
class pnlAuthCPABE(vpss_gui.pnlAuthorityCPABE):
    ...
    def OnDeleteUser(self, event):
        ...
        if usertodel == "":
            ...
            # remove user folder
            # uncomment if want to remove the folder...
    ...
    def OnSetup(self, event):
        ...
        if not first_setup:
            # ----------------------------------------------------------------
            # CREATE NEW AUTHORITY SECRET KEYS
            attr_list = ":".join(ATTRIBUTES)
            ret  = call(["python3", "abe-keygen.py",
            ...
            for filename in filenames:
                ...
                meta, time, encr, sl, gr, pk, policy = get_encrypted_file_metadata(local_ctpath)
...
class pnlUsrCPABE(vpss_gui.pnlClientCPABE):
    ...
    def OnEncrypt(self, event):
        ...
        if readytoenc == 1:
            ...
            time = get_public_time()
            metadata = create_metadata(str(time), self.username, 
    ...
    def OnUpdateFileTable(self, event):
        ...
        for fname in os.listdir(os.path.join(USER_FOLDER, \
            ...
            meta, time, encr, sl, gr, pk, pol = get_encrypted_file_metadata(filename)
            ...
            if meta == NODATA:
                ...
                self.lstEncFilesData.SetStringItem(index, 7, time)

                if int(time) < get_public_time():
    ...
    def OnUpload(self, parent):
        ...
        # check time (to see if the file is obsolete)
